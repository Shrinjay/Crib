...
DB_FIELDS = ['id', 'name', 'business_geodata_id', 'Latitude', 'Longitude']
DB_TYPES = {
    'id': "S",
    'name': 'S',
...
def build_db_gdf(gdf):
    return gdf.drop(columns=[*GEOMETRY_COLS, 'business_geodata'])
...
def build_s3_requests_business_geodata(gdf_s3):
    byte_jsons = list(map(bytes, gdf_s3['business_geodata']))
